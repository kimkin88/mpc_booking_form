...
export const bookingUpdateSchema = z
  .object({
    ...
    use_remaining_for_extra_shots: z.boolean().optional(),
...
export const scheduleEntrySchema = z
  ...
  .superRefine((data, ctx) => {
    const isLiveFormat =
      data.kind === 'live_format' ||
      (data.live_start && data.live_end && (data.day_length == null || data.day_length === ''));

    if (isLiveFormat) {
      if (!data.live_start) {
        ctx.addIssue({
          code: 'custom',
          message: 'Live start date is required',
          path: ['live_start'],
        });
      }
      if (!data.live_end) {
        ctx.addIssue({
          code: 'custom',
          message: 'Live end date is required',
          path: ['live_end'],
        });
      }
      if (!data.format || !String(data.format).trim()) {
        ctx.addIssue({
          code: 'custom',
    ...
    } else if (data.day_length == null || data.day_length === '') {
      ctx.addIssue({
        code: 'custom',
