 ...
 *
 * Anchor date preference: earliest preferred shoot date, else campaign start.
 * Portal lock = Friday strictly before the in-charge period start.
 */
...
/**
 * Earliest preferred shoot date from schedule entries (shoot rows with shoot_date).
 * Ignores blank dates. Live-format-only rows without shoot_date are skipped.
 */
...
/**
 * Anchor for in-charge / lock: earliest shoot date, else campaign start.
 */
export function resolveInChargeAnchor({ campaignStart, scheduleEntries } = {}) {
  return earliestShootDate(scheduleEntries) || (campaignStart ? String(campaignStart).slice(0, 10) : null);
}
 ...
 * @param {string|null} anchorDate — earliest shoot date or campaign start
 */
