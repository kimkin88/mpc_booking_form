...
export function getReminderOffsetsDays() {
  // Default: 1 week before lock, then 3 days, then final day
  const raw = String(process.env.REMINDER_OFFSETS_DAYS || '7,3,1').trim();
  const days = raw
    .split(',')
    .map((v) => Number(String(v).trim()))
    .filter((n) => Number.isFinite(n) && n >= 0);
  return days.length ? [...new Set(days)].sort((a, b) => b - a) : [7, 3, 1];
}
