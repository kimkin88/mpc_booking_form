...
/** Rate-card package lines (price ↔ deliverable). */
...
/** Display label for a shoot day length (0.5 / 1). */
export function dayLengthLabel(dayLength) {
  const opt = DAY_LENGTH_OPTIONS.find((o) => Number(o.value) === Number(dayLength));
  if (opt) return opt.label;
  if (dayLength == null || dayLength === '') return '';
...
export function shootRowsCost(entries = [], rates) {
  return (entries || []).reduce((sum, row) => {
    // Calendar live-format rows (live_start/live_end, no day length) are not shoot costs
